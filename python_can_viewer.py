...
canopen_function_codes = {
    0x000: 'NMT',        # Network management (NMT) node control. The node id should not be added to this value
    0x080: ['SYNC',      # Synchronization (SYNC) protocol. The node id should not be added to this value
            'EMCY'],     # Emergency (EMCY) protocol
    0x100: 'TIME',       # Time (TIME) protocol. The node id should not be added to this value
    ...
    0x580: 'SDO_TX',     # Synchronization Object (SYNC) transmit
    0x600: 'SDO_RX',     # Synchronization Object (SYNC) receive
    0x700: 'HEARTBEAT',  # Network management (NMT) node monitoring
    0x7E4: 'LSS_TX',     # Layer Setting Services (LSS) transmit
    0x7E5: 'LSS_RX',     # Layer Setting Services (LSS) receive
}
...
def draw_can_bus_message(stdscr, msg, sorting=False):
    global ids
    ...
    new_id_added = True if key not in ids else False
    if not sorting:
        if new_id_added:
            # It's a new message ID, so add it to the dict
    ...
    if new_id_added:
        ...
        for i, key in enumerate(sorted(ids.keys())):
            ...
            draw_can_bus_message(stdscr, ids[key][2], sorting=True)
    else:
        ...
        if not msg.is_extended_id and canopen_function_code in canopen_function_codes:
            ...
            if canopen_function_code == 0x080:
                # The SYNC and EMCY uses the same function code, so we simply check if the node ID is non-zero
                function_index = 0 if canopen_node_id == 0 else 1
                canopen_function_code_string = canopen_function_codes[canopen_function_code][function_index]
            elif canopen_node_id != 0 and (canopen_function_code == 0x000 or canopen_function_code == 0x100):
...
def redraw_screen(stdscr):
    ...
    draw_header(stdscr)
    for key in ids.keys():
        draw_can_bus_message(stdscr, ids[key][2])
